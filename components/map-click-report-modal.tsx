        ...
        title: (
          <div className="flex flex-col items-center gap-3 text-center">
            ...
            <span className="text-green-800 text-base">
              {t?.toast?.reportSuccess?.description?.[language] ||
        ...
        title: (
          <div className="flex flex-col items-center gap-3 text-center">
            ...
            <span className="text-red-800 text-base">
              {t?.toast?.reportError?.description?.[language] ||
      ...
      >
        <CardHeader>
          <div className="flex items-start justify-between">
            <div>
              <CardTitle>
                {t?.modals?.clickReport?.title?.[language] || "Report an Issue"}
              </CardTitle>
              <CardDescription>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2">
              <Label htmlFor="issueType">
                {t?.report?.issueType?.[language] || "Issue Type"}
              </Label>
              <select
                id="issueType"
              ...
              >
                <option value="">
                  {t?.report?.issueTypeSelect?.[language] ||
                    "Select issue type"}
                </option>
                <option value="water">
            ...
            <div className="space-y-2">
              <Label htmlFor="location">
                {t?.report?.locationDesc?.[language] || "Location"}
              </Label>
              <Input
                id="location"
                placeholder={
                  t?.report?.locationPlaceholder?.[language] ||
            ...
            <div className="space-y-2">
              <Label htmlFor="description">
                {t?.report?.detailedDesc?.[language] || "Description"}
              </Label>
              <Textarea
                id="description"
                placeholder={
                  t?.report?.detailedPlaceholder?.[language] ||
                  "Describe the issue"
                }
                rows={6}
                required
            ...
            <div className="space-y-2">
              <Label htmlFor="photos">
                {t?.report?.photos?.[language] || "Photos"}
              </Label>
              <label htmlFor="photos" className="block cursor-pointer">
                <div className="border-2 border-dashed border-border rounded-lg p-8 text-center hover:bg-muted/50 transition-colors">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {t?.report?.uploadHint?.[language] || "PNG, JPG up to 10MB"}
                  </p>
                </div>
              </label>
              <input
            ...
            <div className="border-t border-border pt-6">
              <h3 className="font-semibold mb-4">
                {t?.report?.contactInfo?.[language] || "Contact Information"}
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Label htmlFor="reporterName">
                    {t?.report?.nameLabel?.[language] || "Name"}
                  </Label>
                  <Input
                    id="reporterName"
                    placeholder={
                      t?.report?.namePlaceholder?.[language] || "Your name"
                    }
                    required
                ...
                <div className="space-y-2">
                  <Label htmlFor="reporterEmail">
                    {t?.report?.emailLabel?.[language] || "Email"}
                  </Label>
                  <Input
                    id="reporterEmail"
                    type="email"
              ...
              <div className="space-y-2 mt-4">
                <Label htmlFor="reporterPhone">
                  {t?.report?.phoneLabel?.[language] || "Phone"}
                </Label>
                <Input
                  id="reporterPhone"
                  type="tel"
              ...
              >
                {isSubmitting
                  ? t?.modals?.clickReport?.submitting?.[language] ||
                    "Submitting..."
                  : t?.report?.submit?.[language] || "Submit Report"}
              </Button>
              <Button
                type="button"
                variant="outline"
